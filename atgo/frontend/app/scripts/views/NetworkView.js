...
define([
    ...
    var NetworkView = Backbone.View.extend({
        ...
        zoomTo: function (id) {
            ...
            SigmaRenderer.zoomTo(node.displayX, node.displayY, 4);
        ...
        addHiddenEdges: function (pathArray) {
            ...
            _.each(pathArray, function(path2root){
                console.log(path2root);
                children.push(path2root[1]);
            });

            var uniqueChildren = _.unique(children);
            ...
            // Add hidden edges
            var extraEdges = [];
            _.each(uniqueChildren, function (source) {
                var edgeName = source + '-' + target;

                console.log('Adding edge: ' + edgeName);

                if (!edgeNames[edgeName] && source !== undefined) {
        ...
        showPath: function (pathArray) {
            ...
            _.each(pathArray, function (path) {
                ...
                var firstNeighbour = path[1];

                sourceNodes[firstNeighbour] = true;
